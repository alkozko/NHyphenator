...
namespace NHypenator.Tests
{
	...
	public class HypenatorTests
	{
		...
		{
			var hypenator1 = new Hypenator(HypenatePatternsLanguage.EnglishUs);
			Assert.AreEqual("dis&shy;ci&shy;plines", hypenator1.HyphenateText("disciplines"));
		}

		[Test]
		public void HypenateText()
		{
			...
			var hyphenateText = new Hypenator(HypenatePatternsLanguage.EnglishUs, "-",hypenateLastWord:true).HyphenateText(text);
		...
		[Test]
		public void HypenateLastWordOptionFalseTest()
		{
			...
			var hyphenateText = new Hypenator(HypenatePatternsLanguage.EnglishUs, "-", hypenateLastWord: false).HyphenateText(text);
			Assert.AreEqual(expected, hyphenateText);
		}		
		
		[Test]
		public void HypenateLastWordOptionTrueTest()
		{
			var text = @".. consciousness and raise the morale of participants.";
		...
		public void MinLetterCountTest()
		{
			...
			var hyphenateText = new Hypenator(HypenatePatternsLanguage.EnglishUs, "-", minLetterCount: 4).HyphenateText(text);
			Assert.AreEqual(expected, hyphenateText);
		}		
		...
		public void MinLetterCountDontRaiseException()
		{
			...
			var hyphenateText = new Hypenator(HypenatePatternsLanguage.EnglishUs, "-", minLetterCount: 50).HyphenateText(text);
			Assert.AreEqual(expected, hyphenateText);
		}			
		...
		public void MinLetterCountDontRaiseExceptionIfNegativeValue()
		{
			...
			var hyphenateText = new Hypenator(HypenatePatternsLanguage.EnglishUs, "-", minLetterCount: -1).HyphenateText(text);
			Assert.AreEqual(expected, hyphenateText);
		}		
		...
		public void MinWordLengthTest()
		{
			var text = @"disciplines";
			var expected = @"disciplines";
			var hyphenateText = new Hypenator(HypenatePatternsLanguage.EnglishUs, "-", minWordLength: 50).HyphenateText(text);
			Assert.AreEqual(expected, hyphenateText);
		}		
